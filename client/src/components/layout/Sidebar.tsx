...
export function Sidebar({ isMobileOpen, onClose, onLogout }: Props) {
  return (
    <>
      {isMobileOpen && (
        <div
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-55 shrink-0 flex-col border-r border-border bg-surface-secondary transition-transform duration-200 sm:static sm:z-auto sm:w-16 sm:translate-x-0 lg:w-55 ${
          isMobileOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="border-b border-border px-4.5 py-5.5 sm:px-2 sm:py-4 lg:px-4.5 lg:py-5.5">
          <div className="sm:hidden lg:block">
            <div className="font-display text-[20px] italic leading-[1.1] text-pink">Glam AI</div>
            <div className="mt-0.75 text-[10px] uppercase tracking-[0.12em] text-text-secondary">
        ...
        <nav className="flex-1 overflow-y-auto p-2 sm:px-1.5">
          {GROUPS.map((group) => (
            <div key={group}>
              <div className="px-2 pb-1 pt-2 text-[10px] uppercase tracking-widest text-text-secondary sm:hidden lg:block">
                {group}
              </div>
              {routes
                ...
                .map((route) => {
                  ...
                  return (
                    <NavLink
                      ...
                      onClick={onClose}
                      className={({ isActive }) =>
                        `mb-0.5 flex w-full items-center gap-2.25 rounded-md px-2.5 py-2 text-[13px] transition-colors sm:justify-center lg:justify-start ${
                          isActive
