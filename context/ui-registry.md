...
The two action chips diverge from each other on purpose: **"Edit terms ↗"** is a normal `<Chip>` wired through `useChatPrompt()`, matching every other mock-era action chip in the app (`TrendCard`, `ScriptCard`) — it prefills chat with a bespoke per-contract prompt. **"Download PDF"** is the same `<Chip>` component but rendered `disabled` with no `onClick` and no tooltip — there is no real PDF to download until feature 20 builds `services/pdf.ts`, so this follows `CalendarGrid`'s prev/next-chevron precedent exactly (bare `disabled`, no explanatory `title`) rather than omitting the button, faking a download, or inventing a one-off self-explaining-disabled-control pattern. `Chip`'s existing `disabled:*` classes (added in feature 14) already cover this with no component changes needed.
...
**Pattern notes:**
The dashed border (`border-dashed`) is a new motif — no other container in the app uses it — chosen deliberately to read as a drop-zone/upload affordance since there's no existing "file upload" pattern to match and no design-HTML mock for this panel (same situation `ContractCard`/`ScriptCard` were already in). Any future upload control should reuse this dashed-border treatment rather than inventing a third variant. The native `<input type="file">` is styled via Tailwind's `file:` pseudo-element classes rather than a custom-styled trigger button + hidden input, since the plan didn't call for a bespoke drag-and-drop widget — the browser's own file picker is used, only re-skinned.

**2026-09-01 — `<select>` reconciled to `ui-tokens.md`'s documented Input spec (feature 25).** Was `px-2 py-1.5` with no focus state; now `px-3 py-2 focus:border-pink-mid`, matching `SettingsPage`'s `<input type="time">` exactly. This was the one concrete UI gap found during feature 25's audit — no other component's select existed to reconcile against when this was first flagged (feature 21), and none has been added since, so this closes that gap rather than deferring it further.
